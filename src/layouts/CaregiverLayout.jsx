...
export default function CaregiverLayout() {
    ...
    return (
        <div className="min-h-screen bg-[#f4f7f9] flex flex-col font-sans">
            {/* ── Top Bar ── */}
            <header className="h-16 bg-white border-b border-slate-200/80 shadow-sm sticky top-0 z-40 flex items-center px-4 sm:px-6 gap-4">
                {/* Mobile hamburger */}
                <button
                ...
                <div className="flex items-center gap-2.5 cursor-pointer group">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#00677c] to-[#102030] flex items-center justify-center text-white font-bold text-sm shadow-sm">
                        N
                    </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {/* ── Sidebar ── */}
                <aside
                    className={`fixed top-0 left-0 h-full w-64 bg-[#102030] z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-out lg:static lg:translate-x-0 lg:z-auto lg:shadow-none ${
                        sidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
                >
                    {/* Sidebar header (mobile) */}
                    <div className="h-16 flex items-center px-5 border-b border-white/10 lg:hidden shrink-0">
                        <span className="text-white font-bold text-lg">Menu</span>
                        <button
                            className="ml-auto p-1.5 rounded-lg hover:bg-white/10 text-white/70"
                            onClick={() => setSidebarOpen(false)}
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    ...
                    {/* Bottom — back to public */}
                    <div className="p-4 border-t border-white/10">
                        <button
                            onClick={() => navigate("/caregiver")}
                            className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10 transition-all text-sm font-medium"
                        >
